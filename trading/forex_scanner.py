...
class ForexScanner:
    ...
    def _load_lessons(self):
        """Load lessons from DB and extract ALL actionable rules.

        Parses every rule type generated by loss_analyzer:
        - block_symbol:<sym>         → skip this symbol entirely
        - block_category:<cat>       → skip markets in this category
        - block_keyword:<word>       → skip if keyword appears in symbol
        - raise_confidence_threshold:<val> → raise min confidence
        - raise_sentiment_threshold:<val>  → raise sentiment bar
        ...
        - trigger_retrain            → flag for model retraining
        """
        if not self.db:
            return
        ...
        try:
            ...
            max_spread = None  # tightest spread limit from lessons
            min_liquidity = None  # highest liquidity floor from lessons
            min_sources = None
            ...
            for lesson in lessons:
                ...
                # Parse every rule type generated by loss_analyzer
                if rule.startswith("block_symbol:"):
                    blocked_symbols.add(rule.split(":", 1)[1].strip())

                elif rule.startswith("block_category:"):
                    blocked_categories.add(rule.split(":", 1)[1].strip().lower())

                elif rule.startswith("block_keyword:"):
                    blocked_keywords.add(rule.split(":", 1)[1].strip().lower())

                elif rule.startswith("raise_confidence_threshold:"):
                    try:
                        val = float(rule.split(":", 1)[1].strip())
                        confidence_boost = max(confidence_boost, val - 0.50)
                    except ValueError:
                        pass

                elif rule.startswith("raise_sentiment_threshold:"):
                    # Treated as a confidence boost since forex uses confidence
                    try:
                        val = float(rule.split(":", 1)[1].strip())
                        confidence_boost = max(confidence_boost, val * 0.1)
                    except ValueError:
                        pass
            ...
            # Category-based threshold boost on top of explicit rules
            ...
            if blocked_categories:
                applied.append(f"blocked_categories={blocked_categories}")
            if blocked_keywords:
                applied.append(f"blocked_keywords={blocked_keywords}")
            if max_spread is not None:
                applied.append(f"max_spread={max_spread:.1f}")
            if min_liquidity is not None:
                applied.append(f"min_liquidity={min_liquidity:.0f}")
            if confidence_boost > 0:
                applied.append(f"confidence_boost=+{confidence_boost:.0%}")

            logger.info(
                f"LESSONS LOADED: {len(lessons)} lessons, "
                f"categories: {category_counts} | "
                f"rules applied: {', '.join(applied) if applied else 'none'}"
            )
